...
# Load helper functions (winsorize, FF industries, download_parquet,
# batch_run, project_setup, ...).
source("src/utils.R")
...
# Read paths from the .env file (which project_setup() has just created or
# was already there). RAW_DATA_DIR holds the raw WRDS pulls (large, slow
# to refresh); DATA_DIR holds derived parquets produced by scripts 2-4;
# OUTPUT_DIR holds tables and figures.
load_dot_env(".env")
...
#   keyring::key_set("wrds", "username")   # Will prompt for your WRDS username
#   keyring::key_set("wrds", "password")     # Will prompt for your WRDS password
...
# (or simply re-running this script) won't re-pull the file unless you delete
# it first. Matches the JAR data policy expectation that raw inputs are
...
if (file.exists(ccm_path)) {
  ...
  # tictoc::tic() / toc() is a simple way to time how long a download takes.
  tictoc::tic()
...
stocknames_path <- glue("{raw_data_dir}/crsp-stocknames.parquet")

if (file.exists(stocknames_path)) {
  message("Skipping stocknames download — file exists: ", stocknames_path)
} else {
  tictoc::tic()
  ...
  write_parquet(stocknames, stocknames_path)
}
...
# we can use the utility function download_parquet() to do this same approach but
...
# downloaded data. Goes to RAW_DATA_DIR — raw inputs live separately from
# the derived parquets that scripts 2-4 produce.
...
# Download CRSP daily returns (download_parquet function) -------------------------
...
# download_parquet() wrapper from utils.R. It does the same thing — chunked
...
# This download could take 10-15+ minutes. The progress line updates as it goes.
# download_parquet() takes a dbplyr lazy table — same pipe-friendly API as
# Ian Gow's db2pq::lazy_tbl_to_pq(). Build the query with dplyr verbs and
# pipe it into download_parquet() with the output file path.
#
...
#   {raw_data_dir}/ccm-link.parquet         — CCM link table (gvkey -> permno)
#   {raw_data_dir}/crsp-stocknames.parquet  — CRSP SIC codes by permno + date range
#   {raw_data_dir}/fundq-raw.parquet        — Compustat quarterly fundamentals
#   {raw_data_dir}/crsp-dsf-v2.parquet      — CRSP daily stock returns
#   {raw_data_dir}/crsp-index.parquet       — CRSP daily market index returns
#
